...
1. `pg_tblspc/66422/PG_9.3_201306121/16384/12658`: 代表$PGDATA中的相对路径
2. `66422`: 这个对应表空间的oid, 可使用`select spcname from pg_tablespace where oid=66422`查看
3. `16384`: 对应数据库的oid, `select oid from pg_datebase where datname=current_database();`
4. `12658`: 对应表的main fork文件名, 其他fork后面加后缀fsm, vm, init等. 对于超过1GB(系统编译时指定), 文件名后面加.x

数据文件的结构

![数据文件结构](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183030.png)

单个BLOCK的结构

![单个block](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183046.png)

BLOCK头数据结构

| Field               | Type          | Length  | Description                                                  |
| ------------------- | ------------- | ------- | ------------------------------------------------------------ |
| pd_lsn              | xLogRecPtr    | 8 bytes | LSN: next byte after last byte of xlog record for last change to thiis page |
| pd_tli              | uint16        | 2 bytes | TimeLineID of last change(only its lowest 16 bits)           |
| pd_flags            | unit16        | 2bytes  | Flag bits                                                    |
| pd_lower            | LocationIndex | 2 bytes | Offset to start of free space                                |
| pd_upper            | LocationIndex | 2 bytes | Offset to end of free space                                  |
| pd_special          | LocationIndex | 2 bytes | Offset to start of special space                             |
| pd_pagesize_version | unit16        | 2 bytes | Page size and layout version number information              |
| pd_prune_xid        | TransactionId | 4 bytes | Oldest unpruned XMAX on page, or zero if none                |

TUPLE数据结构(表中每一行记录的数据结构)

![TUPE](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183101.png)

TUPLE头部数据结构

| Field       | Type            | Length  | Desctiption                                          |
| ----------- | --------------- | ------- | ---------------------------------------------------- |
| t_xmin      | TransactionId   | 4 bytes | insert XID stamp                                     |
| t_xmax      | TransactionId   | 4 bytes | delete XID stamp                                     |
| t_cid       | CommandId       | 4bytes  | insert add/or delete CID stamp(overlays with t_xvac) |
| t_xvac      | TransactionId   | 4 bytes | XID for VACUUM operation moving a row version        |
| t_ctid      | ItemPointerDate | 6 bytes | current TID of this or newer row version             |
| t_infomask2 | int16           | 2 bytes | number of attributes, plus various flag bits         |
| t_infomask  | uint16          | 2 bytes | various flag bits                                    |
| t_hoff      | uint8           | 1byte   | offset to user data                                  |

使用pg_pageinspect插件可以查看这些数据

![pgpage](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183123.png)

![tuplepage](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183138.png)

## 九. PostgreSQL数据库可靠性

![可靠性](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183157.png)

postgreSQL支持到存储控制器write cache, 该处需要硬件设备支持, 在确认存储控制器有断电保护的情况下, 才能打开.

硬盘的write cache一定要关闭

再上方开启了存储控制器断电保护的后, 数据库的可靠性才能得到保证

![可靠性](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183215.png)

数据可靠性的注意事项

![可靠性](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183239.png)

- `synchronous_commit=on`能保证先提交的事务一定在后提交的事务之前, 如果磁盘IO性能较低, 开启该配置可显著提高数据库写的性能. 可以保证数据的一致性, 但是可能丢掉部分shared buffer中没有flush到磁盘的数据
- `-- fsync=on`, `full_page_write=one` 一定要打开, 不然不能保证数据一致性

![可靠性](https://gitee.com/swang-harbin/pic-bed/raw/master/images/2021/20210222183256.png)