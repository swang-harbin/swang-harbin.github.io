---
title: Spring Data JPA 自动生成主键，创建时间和更新时间
...
1. 自定义的基本 DO 父类
    ...
    * 不能在使用 @Entity 和 @Table 注解
    */
   @MappedSuperclass
   /**
    * 可标注在标注了 @Entity 或 @MappedSuperclass 的类上
    * 指定实体或超类的监听类
   ...
   public class BaseDO implements Serializable {
       /**
        * 唯一 ID
        * @GenericGenerator：Hibernate 通用生成器，此处使用 uuid 策略
       ...
       /**
        * 创建时间
        * @Temporal：只能标注在 java.util.Date 和 java.util.Calendar 类型的属性上，指定映射 Data 和 Calendar 的数据库字段类型
...
3. 在 Application.java 上添加 `@EnableJpaAuditing` 注解，启动 JPA 审核功能
