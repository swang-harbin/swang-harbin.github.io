...
- history -ps arg [arg...]
    ...
    - -w: 保存历史列表到指定的历史文件
    - -n: 读历史文件中未读过的行到历史列表
    - -p: 展开历史参数成多行, 但不保存到历史列表中, ```history -p `hostname` `lscpu` ```, 会执行```hostname```和```lscpu```, 并将执行结果变成多行, 并且不存入到history列表
    - -s: 展开历史参数成一行, 附加在历史列表后, ``` history -s `rm -rf /*` ```, 不会执行```rm -rf /*```, 但会将其保存到history中
